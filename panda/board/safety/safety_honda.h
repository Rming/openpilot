  ...
  if (valid) {
    ...
    if ((addr == 0x201) && (len == 6)) {
      ...
      if ((gas_interceptor > HONDA_GAS_INTERCEPTOR_THRESHOLD) &&
          (gas_interceptor_prev <= HONDA_GAS_INTERCEPTOR_THRESHOLD) && !honda_allow_gas_press) {
        controls_allowed = 0;
      }
    ...
    if (!gas_interceptor_detected) {
      if (addr == 0x17C) {
        ...
        if (gas_pressed && !gas_pressed_prev && !honda_allow_gas_press) {
          controls_allowed = 0;
        }
...
static int honda_tx_hook(CAN_FIFOMailBox_TypeDef *to_send) {
  ...
  int pedal_pressed = (!honda_allow_gas_press && gas_pressed_prev) || (gas_interceptor_prev > HONDA_GAS_INTERCEPTOR_THRESHOLD) ||
