  ...
  if (valid) {
    ...
    if (addr == 0x201) {
      ...
      if ((gas_interceptor > TOYOTA_GAS_INTERCEPTOR_THRESHOLD) &&
          (gas_interceptor_prev <= TOYOTA_GAS_INTERCEPTOR_THRESHOLD && !toyota_allow_gas_press)) {
        controls_allowed = 0;
      }
    ...
    if (addr == 0x2C1) {
      ...
      if ((gas > 0) && (toyota_gas_prev == 0) && !gas_interceptor_detected && !toyota_allow_gas_press) {
        controls_allowed = 0;
      }
      toyota_gas_prev = gas;
    }
