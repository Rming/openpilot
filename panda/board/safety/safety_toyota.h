  ...
  if (valid && (GET_BUS(to_push) == 0)) {
    ...
    if (addr == 0x201) {
      ...
      if ((gas_interceptor > TOYOTA_GAS_INTERCEPTOR_THRSLD) &&
          (gas_interceptor_prev <= TOYOTA_GAS_INTERCEPTOR_THRSLD && !toyota_allow_gas_press)) {
        controls_allowed = 0;
      }
    ...
    if (addr == 0x2C1) {
      ...
      if (gas_pressed && !gas_pressed_prev && !gas_interceptor_detected && !toyota_allow_gas_press) {
        controls_allowed = 0;
      }
